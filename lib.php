...
define('FORMAT_EDUKAV_FILEAREA_IMAGE', 'image');
...
define('FORMAT_EDUKAV_SHOWPROGRESS_HIDE', 2);
define('FORMAT_EDUKAV_PROGRESSFORMAT_COUNT', 1);
define('FORMAT_EDUKAV_PROGRESSFORMAT_PERCENTAGE', 2);
define('FORMAT_EDUKAV_SECTIONNAVIGATION_NONE', 1);
define('FORMAT_EDUKAV_SECTIONNAVIGATION_TOP', 2);
define('FORMAT_EDUKAV_SECTIONNAVIGATION_BOTTOM', 3);
define('FORMAT_EDUKAV_SECTIONNAVIGATION_BOTH', 4);
define('FORMAT_EDUKAV_SECTIONNAVIGATIONHOME_HIDE', '1');
 ...
 * @license     http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_edukav extends format_topics {
    ...
    public function get_course_display(): int {
        ...
        if ($PAGE->pagetype === 'course-section' || $PAGE->pagetype === 'course-view-section-edukav') {
            return $this->get_format_option('subsectionsascards') === FORMAT_EDUKAV_SUBSECTIONS_AS_CARDS
    ...
    public function course_format_options($foreditform = false) {
        ...
        $defaults = get_config('format_edukav');
        ...
        $createselect = function (string $name, array $options, int $default, bool $hashelp = false): array {
            $option = [
                'default' => FORMAT_EDUKAV_USEDEFAULT,
                'type' => PARAM_INT,
                'label' => new lang_string("form:course:$name", 'format_edukav'),
                'element_type' => 'select',
                'element_attributes' => [
                    array_merge(
                        [
                            FORMAT_EDUKAV_USEDEFAULT => new lang_string(
            ...
            if ($hashelp) {
                ...
                $option['help_component'] = 'format_edukav';
            }
        ...
        $section0options = [
            FORMAT_EDUKAV_SECTION0_COURSEPAGE => new lang_string('form:course:section0:coursepage', 'format_edukav'),
            FORMAT_EDUKAV_SECTION0_ALLPAGES => new lang_string('form:course:section0:allpages', 'format_edukav'),
        ];
        ...
        $sectionnavigationoptions = [
            ...
            FORMAT_EDUKAV_SECTIONNAVIGATION_TOP => new lang_string('form:course:sectionnavigation:top', 'format_edukav'),
            FORMAT_EDUKAV_SECTIONNAVIGATION_BOTTOM => new lang_string('form:course:sectionnavigation:bottom', 'format_edukav'),
            FORMAT_EDUKAV_SECTIONNAVIGATION_BOTH => new lang_string('form:course:sectionnavigation:both', 'format_edukav'),
        ];
        ...
        $sectionnavigationhomeoptions = [
            FORMAT_EDUKAV_SECTIONNAVIGATIONHOME_HIDE =>
                new lang_string('form:course:sectionnavigationhome:hide', 'format_edukav'),
            FORMAT_EDUKAV_SECTIONNAVIGATIONHOME_SHOW =>
                new lang_string('form:course:sectionnavigationhome:show', 'format_edukav'),
        ];
        ...
        $showprogressoptions = [
            FORMAT_EDUKAV_SHOWPROGRESS_SHOW => new lang_string('form:course:showprogress:show', 'format_edukav'),
            FORMAT_EDUKAV_SHOWPROGRESS_HIDE => new lang_string('form:course:showprogress:hide', 'format_edukav'),
        ];

        $options['showprogress'] = $createselect('showprogress', $showprogressoptions, $defaults->showprogress);

        $progressformatoptions = [
            FORMAT_EDUKAV_PROGRESSFORMAT_COUNT => new lang_string('form:course:progressformat:count', 'format_edukav'),
            FORMAT_EDUKAV_PROGRESSFORMAT_PERCENTAGE => new lang_string('form:course:progressformat:percentage', 'format_edukav'),
        ];

        $options['progressformat'] = $createselect('progressformat', $progressformatoptions, $defaults->progressformat);

        $subsectionoptions = [
            FORMAT_EDUKAV_SUBSECTIONS_AS_CARDS => new lang_string('form:course:subsectionsascards:cards', 'format_edukav'),
            FORMAT_EDUKAV_SUBSECTIONS_AS_ACTIVITIES => new lang_string('form:course:subsectionsascards:activity', 'format_edukav'),
        ];
    ...
    public function section_format_options($foreditform = false) {
        ...
        $options['showsummary'] = [
            'default' => FORMAT_EDUKAV_USEDEFAULT,
            'type' => PARAM_INT,
            'label' => new lang_string('form:course:showsummary', 'format_edukav'),
            'element_type' => 'select',
            'element_attributes' => [
                array_merge(
                    [
    ...
    public function create_edit_form_elements(&$mform, $forsection = false): array {
        ...
        if ($this->course_has_grid_images() && !$forsection) {
            $elements[] = $mform->addElement(
                'checkbox',
                'importgridimages',
                get_string('form:course:importgridimages', 'format_edukav')
            );
            $mform->addHelpButton('importgridimages', 'form:course:importgridimages', 'format_edukav');
        }

        $defaultshowprogress = get_config('format_edukav', 'showprogress');
        $hiddenvalues = [ FORMAT_EDUKAV_SHOWPROGRESS_HIDE ];

        if ($defaultshowprogress == FORMAT_EDUKAV_SHOWPROGRESS_HIDE) {
            $hiddenvalues[] = FORMAT_EDUKAV_USEDEFAULT;
        }
    ...
    public function update_course_format_options($data, $oldcourse = null): bool {
        ...
        foreach ($gridimages as $gridimage) {
            ...
            try {
                $newfile = $storage->create_file_from_storedfile([
                    ...
                    'component' => 'format_edukav',
                    'filearea' => FORMAT_EDUKAV_FILEAREA_IMAGE,
            ...
            try {
                ...
                if (!$hasdisplayedresizeerror) {
                    notification::add(get_string('editimage:resizefailed', 'format_edukav'), notification::WARNING);
        ...
        notification::add(
            get_string('editimage:imported', 'format_edukav', $added),
    ...
    public function get_format_option(string $name, $section = null) {
        ...
        if ($value != FORMAT_EDUKAV_USEDEFAULT) {
            return $value;
        }
        ...
        if (!is_null($section)) {
            ...
            if ($coursedefaults->$name != FORMAT_EDUKAV_USEDEFAULT) {
    ...
    public function get_default_section_name($section) {
        ...
        return get_string('section:default', 'format_edukav', $section->section);
    }

    /**
     * format_topics::get_view_url() returns the course URL with an HTML anchor pointing to the selected
     * section, if there is one. format_edukav always uses single section per page mode, so we always want
    ...
    public function resize_card_image($section): void {
        ...
        $tempfilepath = $originalimage->copy_content_to_temp('format_edukav', 'sectionimage_');

        $resized = resize_image($tempfilepath, null, 500, false);

        if (!$resized) {
            throw new moodle_exception('failedtoresize', 'format_edukav');
        }
 ...
 * @throws dml_exception
 */
function format_edukav_inplace_editable($itemtype, $itemid, $newvalue) {
    global $DB, $CFG;
 ...
 * @throws moodle_exception
 */
function format_edukav_pluginfile(stdClass $course,
